...
export const BrowserExportModal: React.FC<BrowserExportModalProps> = ({
  ...
  const rawTextExport = [
    ...
    `Total Signals: ${profile.summary.totalSignalsEvaluated}`,
    `Confirmed Fingerprinting Exposures: ${profile.summary.exposedHardwareSignals}`,
    `Privacy Protections: ${profile.summary.privacyProtectionsActive}`,
    '',
    ...
    '',
    '--- PRIVACY PROTECTIONS ---',
    `Global Privacy Control (GPC): ${(profile.groups.PRIVACY_PROTECTIONS?.data as any)?.globalPrivacyControl === true ? 'ACTIVE' : (profile.groups.PRIVACY_PROTECTIONS?.data as any)?.globalPrivacyControl === false ? 'INACTIVE' : 'NOT MEASURED'}`,
    `Do Not Track (DNT): ${(profile.groups.PRIVACY_PROTECTIONS?.data as any)?.doNotTrack === true ? 'ACTIVE' : (profile.groups.PRIVACY_PROTECTIONS?.data as any)?.doNotTrack === false ? 'INACTIVE' : 'NOT MEASURED'}`,
    '========================================================',
  ].join('\n');
