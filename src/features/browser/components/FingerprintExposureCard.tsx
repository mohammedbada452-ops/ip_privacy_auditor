...
export const FingerprintExposureCard: React.FC<{ profile: BrowserProfile }> = ({ profile }) => {
  ...
  return <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5" aria-labelledby="fingerprint-exposure-title"><div className="flex items-start gap-3"><div className="w-9 h-9 rounded-xl bg-fuchsia-500/10 border border-fuchsia-500/20 text-fuchsia-300 flex items-center justify-center"><Fingerprint className="w-4 h-4" aria-hidden="true"/></div><div><h2 id="fingerprint-exposure-title" className="text-sm font-bold text-slate-100">{c.title}</h2><p className="text-xs text-slate-500 mt-1">{c.subtitle}</p></div></div><div className="mt-5 grid grid-cols-3 gap-3"><div className="rounded-xl bg-slate-950/50 border border-slate-800 p-3"><div className="text-[10px] text-slate-500">{c.bits}</div><div className="text-xl font-bold text-slate-100 mt-1">{bits} <span className="text-xs text-slate-500">bits</span></div></div><div className="rounded-xl bg-slate-950/50 border border-slate-800 p-3"><div className="text-[10px] text-slate-500">{c.distinct}</div><div className="text-xl font-bold text-fuchsia-300 mt-1">{distinctLabel}</div></div><div className="rounded-xl bg-slate-950/50 border border-slate-800 p-3"><div className="text-[10px] text-slate-500">{c.measured}</div><div className="text-xl font-bold text-slate-100 mt-1">{measured}</div></div></div><div className="mt-4 flex gap-2 text-[11px] leading-5 text-slate-500"><Info className="w-3.5 h-3.5 mt-0.5 shrink-0 text-slate-600"/>{c.note}</div></section>;
};
